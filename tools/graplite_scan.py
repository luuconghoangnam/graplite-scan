...
CS_COMMAND_ASSIGN_RE = re.compile(
    r'\b(?P<name>[A-Za-z_][A-Za-z0-9_]*Command)\s*=\s*new\s+(?P<kind>[A-Za-z_][A-Za-z0-9_]*)',
    re.M,
)
...
def render_blast_map(
    ...
    def build_app_feature_flow_map() -> List[Tuple[str, List[str], List[str], List[str]]]:
        ...
        def candidate_frontend_files() -> List[str]:
            ...
            def is_app_router_file(file_path: str) -> bool:
                lower = file_path.lower()
                return any(token in lower for token in ('/app/', '/pages/', '/routes/')) and any(
                    marker in lower for marker in ('page.', 'layout.', 'template.', 'loading.', 'error.', 'not-found.', 'providers.')
                )

            def add_candidate(child_rel: str) -> None:
            ...
            for file_path, deps in edges.items():
                ...
                if is_app_router_file(file_path):
                    score += 3
            ...
            for _neg_score, file_path in priority_files[:10]:
                ...
                candidates.append(file_path)
                if is_app_router_file(file_path):
                    add_app_router_chain(file_path)
    ...
    def build_frontend_shared_surface_map() -> List[Tuple[int, str, List[str], List[str], List[str]]]:
        ...
        for file_path in set(edges.keys()) | set(reverse_edges.keys()):
            ...
            if not consumers:
                continue
            page_consumers = [c for c in consumers if any(token in c.lower() for token in ('/pages/', '/routes/', '/app/', '/features/', 'page.', 'layout.', 'template.', 'loading.', 'error.', 'not-found.', 'screen.', 'view.'))]
            ...
            if '/layouts/' in lower or '/providers/' in lower or '/shell/' in lower or any(token in lower for token in ('layout.', 'template.', 'loading.', 'error.', 'not-found.')):
                score += 3
    ...
    def build_desktop_flow_map() -> List[Tuple[int, str, Optional[str], List[str], List[str], List[str], List[str]]]:
        ...
        for view_path in candidate_views:
            ...
            def append_files_containing(bucket: List[str], names: Iterable[str], candidates: Iterable[str]) -> None:
                wanted = [name.split('.')[-1].strip() for name in names if name and name.strip()]
                for candidate in candidates:
                    ...
                    for wanted_name in wanted:
                        if not wanted_name:
                            continue
                        if re.search(r'\b' + re.escape(wanted_name) + r'\b', txt_candidate):
                            if candidate not in bucket:
                                bucket.append(candidate)
                            break
